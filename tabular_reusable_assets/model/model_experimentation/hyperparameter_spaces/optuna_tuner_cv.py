...
from tabular_reusable_assets.model.model_utils import create_group_folds, cv_prediction, plot_learning_curve
...
class TuningConfig:
    ...
    cv: BaseCrossValidator = StratifiedGroupKFold(n_splits=5, random_state=None, shuffle=False)
    scorer_metric: str = "roc_auc"
    cv_agg_method: str = "mean"  # Method to aggregate CV scores: 'mean', 'median', 'min', 'max'
    cv_error_handling: str = "raise"
    custom_cv_scoring: Optional[Callable] = None
    custom_cv_scoring_params: Optional[Dict] = None
...
class ModelHyperParameterTunerCV(BaseModelTuner):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.input_cols = list(kwargs.get("input_cols", input_cols))
        # Setup logging
        self._setup_logging()
        # Initialize study
    ...
    def init_folds(self):
        self.model_master_df = create_group_folds(
            cv=self.tuning_config.cv,
        ...
        try:
            ...
            if self.tuning_config.custom_cv_scoring:
                _, cv_summary_stats = self.tuning_config.custom_cv_scoring(
                    data=self.model_master_df,
                    model=model,
                    feature_columns=self.input_cols,
                    target_column=self.target_column,
                    fold_column="fold",
                    scoring=[self.tuning_config.scorer_metric],
                ...
                if score > self.best_score:
                    ...
                    self.best_params = cv_summary_stats["model"].get_params()
                # Store best iteration
                trial.set_user_attr("best_iteration", cv_summary_stats["best_iteration"])

            else:
                _, cv_summary_stats = cv_prediction(
                    data=self.model_master_df,
                    model=model,
                    feature_columns=self.input_cols,
                    target_column=self.target_column,
                ...
                # Store best iteration
                trial.set_user_attr("best_iteration", cv_summary_stats["model"].best_iteration)
...
if __name__ == "__main__":
    from tabular_reusable_assets.model.model_utils import two_stage_cv_prediction
    ...
    config = TuningConfig(
        ...
        fit_params={},  # when using CV for XGBoost you cannot use early_stopping, eval_set
        cv=StratifiedGroupKFold(n_splits=5, random_state=None, shuffle=False),
        custom_cv_scoring=two_stage_cv_prediction,
        custom_cv_scoring_params={"early_stopping_strategy": "median"},
        scorer_metric="roc_auc",
        cv_agg_method="mean",  # Method to aggregate CV scores: 'mean', 'median', 'min', 'max'
        cv_error_handling="raise",
    ...
    experiment = ModelHyperParameterTunerCV(
        tuning_config=config,
        model_master_df=pd.concat([X_train, y_train], axis=1),
