...
from tabular_reusable_assets.utils.data_drift_helper import DataDriftHelper
...
def test_psi_calculation_table(titanic_dataset):
    ...
    indiv_psi_table = DataDriftHelper.calculate_feat_psi_verbosed(
...
def test_psi_calculation(titanic_dataset):
    ...
    indiv_psi_table = DataDriftHelper.calculate_feat_psi(left_df=data, right_df=data2, feat="qAge")
...
def test_psi_good_and_bad_dummy():
    ...
    assert DataDriftHelper.calculate_feat_psi(df1, df2, feat="col1")["sum_psi"].iloc[0] == 0
    assert np.allclose(DataDriftHelper.calculate_feat_psi(df1, df3, feat="col1")["sum_psi"].iloc[0], 0.810795895439714)
...
def test_binner_with_psi(titanic_dataset):
    ...
    psi = DataDriftHelper.calculate_feat_psi(train2, val2, feat="Age_bin")
